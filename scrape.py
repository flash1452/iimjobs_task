...
for row in result_set:
    ...
    try:
        ...
        if row[1] == "MegasoftInfo":
            company1.comp1_crawling(row[0], row[3], company_posting_name, last_updated, new_posting_details)
        else:
            if row[1] == "WeMakeScholars" :
                company2.comp2_crawling(row[0], row[3], company_posting_name, last_updated, new_posting_details)
